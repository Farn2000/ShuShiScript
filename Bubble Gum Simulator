...
b:AddSwitch("Auto Farm",function(bool)
    af = bool
end)
b:AddSwitch("Auto Sell",function(bool)
    as = bool
end)
local Egg = b:AddDropdown("Eggs",function(a)
    egg = a
end)
...
b:AddLabel('Must stand in front of the egg')
b:AddSwitch("Auto Open Eggs",function(bool)
    ao = bool
end)

b:AddButton("Unlock All Islands",function()
    ...
    while wait() do
        if ao then
            if workspace.Eggs:FindFirstChild(egg) and (workspace.Eggs[egg].Hotkey.Position - plr.Character.HumanoidRootPart.Position).Magnitude < 15 then
                game:GetService("ReplicatedStorage").NetworkRemoteEvent:FireServer("PurchaseEgg",egg)
            end
        end
    end
end)
